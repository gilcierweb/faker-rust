...
pub fn backward(days: Option<i64>, from: Option<&str>, to: Option<&str>) -> String {
...
pub fn forward(days: Option<i64>, from: Option<&str>, to: Option<&str>) -> String {
    let days_forward = days.unwrap_or(365);
    let today = Utc::now().date_naive();
    let target = today + Duration::days(days_forward);
    target.format("%Y-%m-%d").to_string()
}
...
pub fn between(from: &str, to: &str) -> String {
    ...
    let target = start + Duration::days(days_offset);
    target.format("%Y-%m-%d").to_string()
}
...
pub fn birthday(min_age: Option<i32>, max_age: Option<i32>) -> String {
    ...
    let days_offset = config.rand_range(0, days_in_year) as i64;
    ...
    let target = base_date + Duration::days(day_of_year - 1);

    target.format("%Y-%m-%d").to_string()
}

/// Generate a random date in a specific period
pub fn in_date_period(start_date: &str, end_date: &str, format: Option<&str>) -> String {
    between(start_date, end_date)
}

/// Generate a random date on a specific day of week
pub fn on_day_of_week_between(day_of_week: &str, from: &str, to: &str) -> String {
